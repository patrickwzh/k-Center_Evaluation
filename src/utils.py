...
def load_data(data_path):
    ...
    # Store distances
    data.allDists = sorted(
        set(dist[i][j] for i in range(data.n) for j in range(i + 1, data.n))
    )
...
def compute_results(file_path):
    ...
    for key, value in results.items():
        ...
        make_span_std = math.sqrt(
            sum((x - make_span_avg) ** 2 for x in make_span_list) / len(make_span_list)
        )
        time_avg = sum(time_list) / len(time_list)
        time_std = math.sqrt(
            sum((x - time_avg) ** 2 for x in time_list) / len(time_list)
        )
        final_results[key] = {
...
def plot_make_span(data):
    ...
    for alg in algorithms:
        ...
        y[alg] = np.array(
            [
                data.get((alg, str(i)), {}).get("make_span_avg", None)
                for i in range(1, 41)
            ],
            dtype=float,
        )
        ...
        yerr[alg] = np.array(
            [
                data.get((alg, str(i)), {}).get("make_span_stddev", None)
                for i in range(1, 41)
            ],
            dtype=float,
        )
...
def plot_time(data):
    ...
    for alg in algorithms:
        ...
        # y[alg] = np.array(y[alg], dtype=float)
        y[alg] = np.array(
            [data.get((alg, str(i)), {}).get("time_avg", None) for i in range(1, 41)],
            dtype=float,
        )
        yerr[alg] = []
        for i in range(1, 41):
        ...
        yerr[alg] = np.array(
            [
                data.get((alg, str(i)), {}).get("time_stddev", None)
                for i in range(1, 41)
            ],
            dtype=float,
        )
