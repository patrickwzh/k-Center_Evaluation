            ...
            for data_path in data_files[algorithm]:
                if algorithm != "Scr_p":
                    for run in range(1, 4):
                        ...
                        result_file.write(
                            f"Algorithm: {algorithm}, Data: {data_path}, Run: {run}\n"
                        )
                        result_file.write(
                            f"- Make-span: {number}, Time: {time_lapse}, Result: {result_list}\n\n"
                        )
                else:
                    for p in range(2, 9):
                        for run in range(1, 4):
                            (result_list, number), time_lapse = eval(
                                algorithm, data_path, p
                            )
                            result_file.write(
                                f"Algorithm: {algorithm}, Data: {data_path}, p: {p}, Run: {run}\n"
                            )
                            result_file.write(
                                f"- Make-span: {number}, Time: {time_lapse}, Result: {result_list}\n\n"
                            )
